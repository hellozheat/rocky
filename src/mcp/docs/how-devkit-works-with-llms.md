...
MCP devkit is a **handbook + tooling** for the AI in your editor—not a separate code generator that replaces the model.

## What you should expect

When devkit is connected, the model should:

1. Call **`devkit`** → `list_handbook` (or use prompt **`devkit-start-task`**).
2. Read the matching **agents** and **rules** for the task (e.g. `react-developer.md`, `vitest-writer.md`, `code-reviewer.md`).
...
| User goal | How |
| --------- | --- |
| Write better code | Handbook rules + agents (`react-developer`, `human-readable-code`, …) — **you** apply them while editing |
| Split / refactor components | Same — follow `react-developer` / `codebase-conventions`; use `change_scope_analyzer` for blast radius |
...
**Read the handbook, then implement in the repo; use devkit tools to discover, test, and gate—not instead of coding.**
